...
def display_pagination(total_pages):
    ...
    with col2:
        ...
        if 'new_page' not in st.session_state:
            st.session_state.new_page = st.session_state.current_page

        # 使用 number_input 來選擇頁碼
        ...
        if new_page != st.session_state.current_page:
            st.session_state.new_page = new_page
...
def main():
    ...
    # 在主循環中檢查頁面是否變化
    if 'page_changed' in st.session_state and st.session_state.page_changed:
